...
let searchByName = async (ctx, next) => {
  let res = await sequelize.query(
    "select * from user where nickName  like '%" + ctx.query.name + "%'",
    {
      type: sequelize.QueryTypes.SELECT,
    }
  )
...
let getAppointList = async (ctx, next) => {
  ...
  let res = await sequelize.query(
    'select * from `appoint` limit ' + page + ',5',
    {
      type: sequelize.QueryTypes.SELECT,
    }
  )
...
let getfoodList = async (ctx, next) => {
  let count = await sequelize.query(
    'select count(*) as count  from foods where status=1',
    {
      type: sequelize.QueryTypes.SELECT,
    }
  )
  let res = await sequelize.query(
    'select * from foods where status=1  order by id desc  limit ' +
      ctx.query.page +
      ',' +
      ctx.query.pageSize,
    {
...
let addFood = async (ctx, next) => {
  ...
  let crb = ctx.request.body
  let res = await sequelize.query(
    'INSERT INTO `foods` (`type`,`name`,`price`,`img`,`enName`,`status`,`tem`,`size`,`desc`) VALUES' +
      " ('" +
      crb.type +
      "','" +
      crb.name +
...
let getUserInfo = async (ctx, next) => {
  let res = await sequelize.query(
    "select * from user where userName='" + ctx.query.userName + "'",
    {
      type: sequelize.QueryTypes.SELECT,
    }
  )
...
let deleteUser = async (ctx, next) => {
  let res = await sequelize.query(
    "DELETE FROM `users` WHERE `username` = '" + ctx.query.username + "'",
    {
...
let deleteFood = async (ctx, next) => {
  let res = await sequelize.query(
    "UPDATE  `foods` SET `status`=0  WHERE `id` = '" + ctx.query.id + "'",
    {
      type: sequelize.QueryTypes.UPDATE,
    }
  )
...
let addMenu = async (ctx, next) => {
  ...
  let crb = ctx.request.body
  let res = await sequelize.query(
    "INSERT INTO `menu` (`type`) VALUES ('" + crb.type + "')",
    {
      type: sequelize.QueryTypes.INSERT,
    }
  )
...
let getImgNum = async (ctx, next) => {
  let res = await sequelize.query(
    "select `img` from  `foods`  where type='" +
      ctx.query.type +
...
let searchFoodsByName = async (ctx, next) => {
  let res = await sequelize.query(
    "select * from `foods` where name  like '%" + ctx.query.name + "%'",
    {
      type: sequelize.QueryTypes.SELECT,
    }
  )
...
let overOrder = async (ctx, next) => {
  let res = await sequelize.query(
    "UPDATE  `order` SET `status`=2  where orderId='" + ctx.query.orderId + "'",
    {
      type: sequelize.QueryTypes.UPDATE,
    }
  )
...
let refundOrder = async (ctx, next) => {
  let res = await sequelize.query(
    "UPDATE  `order` SET `status`=0  where orderId='" + ctx.query.orderId + "'",
    {
      type: sequelize.QueryTypes.UPDATE,
    }
  )
...
let printDay = async (ctx, next) => {
  ...
  let orderNumber = await sequelize.query(
    "select count(*) as count  from `order`  where  status =2 AND time BETWEEN '" +
      dayBegin +
      "' AND '" +
      dayEnd +
      "' ",
