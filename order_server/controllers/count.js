...
let getUserDayCount = async (ctx, next) => {
  ...
  let res = await sequelize.query(
    "select count(*) as count  from user where  create_time BETWEEN '" +
      dayBegin +
      "' AND '" +
      dayEnd +
      "' ",
...
let getorderAllCount = async (ctx, next) => {
  let res = await sequelize.query(
    'select count(*) as count  from `order` where model=0',
    {
      type: sequelize.QueryTypes.SELECT,
    }
  )
...
let getorderDayCount = async (ctx, next) => {
  ...
  let dayEnd = moment(day).format('YYYY-MM-DD 23:59:59')
  let res = await sequelize.query(
    "select count(*) as count  from `order`  where  model =0 AND time BETWEEN '" +
      dayBegin +
      "' AND '" +
      dayEnd +
      "' ",
...
let getAppointAllCount = async (ctx, next) => {
  let res = await sequelize.query(
    'select count(*) as count  from `order` where model=1',
    {
      type: sequelize.QueryTypes.SELECT,
    }
  )
...
let getAppointDayCount = async (ctx, next) => {
  ...
  let dayEnd = moment(day).format('YYYY-MM-DD 23:59:59')
  let res = await sequelize.query(
    "select count(*) as count  from `order` where model =1  AND  time BETWEEN '" +
      dayBegin +
      "' AND '" +
      dayEnd +
      "' ",
