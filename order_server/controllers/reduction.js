...
let getUserCanUseReductionList = async (ctx, next) => {
  ...
  let res = await sequelize.query(
    'select * from `reduction` where status = 1',
    {
  ...
  for (let i = 0; i < res.length; i++) {
    ...
    let isR = await sequelize.query(
      "select * from `mycut` where reduction='" +
        res[i].id +
        "' and openid='" +
        ctx.query.openid +
        "'",
...
let getCut = async (ctx, next) => {
  ...
  let res = await sequelize.query(
    "INSERT INTO `mycut` (`openid`,`reduction`,`status`,`getTime`) VALUES ('" +
      crb.openid +
      "','" +
      crb.reduction +
      "',0,'" +
...
let getCutList = async (ctx, next) => {
  ...
  //获取我的所有优惠券
  let res = await sequelize.query(
    "select * from `mycut` where openid = '" + ctx.query.openid + "'",
    {
      type: sequelize.QueryTypes.SELECT,
    }
  )
  for (let i = 0; i < res.length; i++) {
    let iuy = await sequelize.query(
      "select * from `reduction` where id = '" +
        res[i].reduction +
        "' and endDate >'" +
        time_now +
        "'",
    ...
    for (let i = 0; i < iuy.length; i++) {
      if (iuy[i].type == 2) {
        let result = await sequelize.query(
          "select * from `foods` where id='" + iuy[i].rule + "'",
          {
            type: sequelize.QueryTypes.SELECT,
          }
        )
...
let useCut = async (ctx, next) => {
  let time = moment().format('YYYY-MM-DD HH:mm:ss')
  let res = await sequelize.query(
    "UPDATE  `mycut` SET `status`=1, `useTime`='" +
      time +
      "'  where id='" +
      ctx.query.id +
      "'",
...
let getAllReduction = async (ctx, next) => {
  //获取所有优惠活动
  let res = await sequelize.query(
    'select * from `reduction` order by id desc',
    {
      type: sequelize.QueryTypes.SELECT,
    }
  )
  ...
  for (let i = 0; i < res.length; i++) {
    let all = await sequelize.query(
      "select count(*) as count  from `mycut` where reduction='" +
        res[i].id +
        "'",
      {
        type: sequelize.QueryTypes.SELECT,
    ...
    if (res[i].type == 2) {
      let result = await sequelize.query(
        "select * from `foods` where id='" + res[i].rule + "'",
        {
          type: sequelize.QueryTypes.SELECT,
        }
      )
...
let addReduction = async (ctx, next) => {
  ...
  if (crb.type == 1) {
    ...
    //检查当前是否还有未结束的满减活动
    let res = await sequelize.query(
      "select * from `reduction` where type='" + crb.type + "' and  status=1",
      {
        type: sequelize.QueryTypes.SELECT,
      }
    )
    if (res.length > 0) {
      ...
      let addReduction = await sequelize.query(
        'INSERT INTO `reduction` (`type`,`typeDes`,`rule`,`cut`,`startDate`,`endDate`,`status`,`img`) VALUES ' +
          "('" +
          crb.type +
          "','" +
          crb.name +
          ...
          crb.cut +
          "','" +
          timeF(crb.timeRange[0]) +
          "','" +
          timeF(crb.timeRange[1]) +
          "',0,'" +
          crb.imageUrl +
          "')",
        {
